...
const MovieDetailsPage = () => {
  ...
  const { data } = useSWR([API.getVehicleDetails(movieId), token], fetcher);

  if (!data) return null;
  const { backdropPath, path, title, description, price } = data.data;
  // const { backdrop_path, poster_path, title, genres, overview } = data;

  // ----------------------------------------------------------------------------------------------------------------------------------------------
  
  const fetchData = async () => {
    const response = await axios.post(API.postOrder(), payload, { headers: { "Authorization": `Bearer ${token}` } }).then((res) => res.data);
    return response;
  };
